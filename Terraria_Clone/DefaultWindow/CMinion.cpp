...
void CMinion::Status_Check()
{
	...
	if ((CTileMgr::Get_Instance()->Get_Tile(m_vecTile[0] + TILEX)->Get_Option(CTile::TILE_BLOCK) == 0) &&
		(CTileMgr::Get_Instance()->Get_Tile(m_vecTile[1] + TILEX)->Get_Option(CTile::TILE_BLOCK) == 0))
	{
		m_bAir = true;
	}
