...
CNPC_Merchant::CNPC_Merchant()
{
	m_eNPCID = NPC_MERCHANT;
}
...
void CNPC_Merchant::Initialize()
{
	...
	m_pFrameKey = L"N02_MERCHANT_RIGHT";

	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/NPC/N02_MERCHANT_LEFT.bmp", L"N02_MERCHANT_LEFT");
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/NPC/N02_MERCHANT_RIGHT.bmp", L"N02_MERCHANT_RIGHT");
}

int CNPC_Merchant::Update()
{
	Status_Check();
...
void CNPC_Merchant::Late_Update()
{
	Pattern_Change();
...
void CNPC_Merchant::Render(HDC hDC)
{
...
void CNPC_Merchant::Release()
{
}

void CNPC_Merchant::Status_Check()
{
	if (m_iCurHP <= 0)
	{
		m_bDead = true;
	...
	{
		m_pFrameKey = L"N02_MERCHANT_LEFT";
	}
