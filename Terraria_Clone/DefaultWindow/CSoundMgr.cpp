...
void CSoundMgr::PlaySoundQuick(const TCHAR* pSoundKey, CHANNELID eID,
	float fVolume, int iRepeatCount, int iDelayMs)
{
	...
	FMOD_System_PlaySound(m_pSystem, FMOD_CHANNEL_FREE,
		iter->second, FALSE, &pChannel);
	if (pChannel)
		FMOD_Channel_SetVolume(pChannel, fVolume);

	// 나머지는 스레드나 타이머로 처리 (간단한 방법)
	// 또는 메인 게임 루프에서 처리하도록 큐에 저장
