...
void CNPC_Nurse::Initialize()
{
	...
	m_pFrameKey = L"N01_Nurse_RIGHT";

	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/NPC/N03_Nurse_LEFT.bmp", L"N03_Nurse_LEFT");
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/NPC/N03_Nurse_RIGHT.bmp", L"N03_Nurse_RIGHT");
}

int CNPC_Nurse::Update()
{
	Status_Check();
...
void CNPC_Nurse::Render(HDC hDC)
{
...
void CNPC_Nurse::Release()
{
}

void CNPC_Nurse::Status_Check()
{
	if (m_iCurHP <= 0)
	{
		m_bDead = true;
	...
	if (cosf(m_fAngle) >= 0)
	{
		m_pFrameKey = L"N03_Nurse_RIGHT";
	}
	else
	{
		m_pFrameKey = L"N03_Nurse_LEFT";
	}
