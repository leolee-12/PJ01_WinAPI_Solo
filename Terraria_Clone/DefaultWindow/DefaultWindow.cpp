						...
						_In_ int       nCmdShow)
{
	...
	if (SUCCEEDED(hr) && lpDD)
	{
		lpDD->SetCooperativeLevel(nullptr, DDSCL_NORMAL);
		lpDD->Release();
	}
	// 전역 문자열을 초기화합니다.
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEWHEEL:
	{
		...
		if ((zDelta > 0) && (!CKeyMgr::Get_Instance()->Get_WheelUp()))
			CKeyMgr::Get_Instance()->Set_WheelUp(true);
		else if ((zDelta < 0) && (!CKeyMgr::Get_Instance()->Get_WheelDown()))
			CKeyMgr::Get_Instance()->Set_WheelDown(true);
	}
	break;
